...
string cacheCheck(string str, char requestType) {
    ...
    //cout << str << requestType << endl;

    // read proxyCache.txt 
	while (file_pointer >> ip >> domain) {
        //cout << ip << " " << domain << endl;
        // cout << str.length() << " " << domain.length() << endl;
        ...
        //cout<<ip<<str<<ip.length()<<str.length();
        if(requestType == '2') {
            // cout << "HI ";
            //debug(str, ip, str.length(), ip.length());
...
string requestToDNS(char buffer[]){
    ...
    if(dns_response_buffer[0] == '3') { 
        ...
        if(cacheData.size() < 3) {  // cache is not full
    ...
    multipleClients:
		...
		if(pid == 0) {
			...
			// Type 2 request: ip address -> domain name
                ...
                // cout << IP << read_buffer[0] << endl;
                //cout << mapped_domain << "value from main\n";
                // cache miss
                if(mapped_domain == "") {
                    fprintf(stderr, "Cache miss. Connecting with DNS Server...\n");
